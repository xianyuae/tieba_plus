...
var idDatabase = function(name, desc, size){
	...
	this.CreateIndex = function(name, index, columns){
		var tname = TABLE_PREFIX !== "" ? TABLE_PREFIX + "_" + name : name;
		db.transaction(function(ta){
			ta.executeSql('CREATE INDEX IF NOT EXISTS %1 ON %2 (%3)'.arg(index).arg(tname).arg(columns));
		});
	}
	...
	this.Select = function(tname, model, limit){
		var push = Array.isArray(model) ? "push" : "append";
		var sql = 'SELECT * FROM %1 ORDER BY ts DESC'.arg(tname);
		if(limit > 0)
			sql += ' LIMIT %1'.arg(limit);
