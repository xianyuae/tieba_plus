...
BasePage {
	...
	QtObject{
		...
		function _Request(url)
		{
			...
			if(!nu)
			{
				controller._ShowMessage(qsTr("Invalid url"));
				return;
			}
			if(input.text != nu) input.text = nu;
			if(/bilibili\.com\/opus\//i.test(nu) || /bilibili\.com\/read\/cv/i.test(nu))
			{
				controller._OpenArticlePage(nu);
				return;
			}
			webpage.url = nu;
		}
	...
	Header{
		...
		TextField{
			...
			z: 1;
			clip: true;
			placeholderText: qsTr("Input url");
