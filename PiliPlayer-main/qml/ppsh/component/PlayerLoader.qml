...
Item{
	...
	property variant __playerComponent: null;
	signal exit;
	signal stop;
	signal menu;
	...
	function _Load(aid, cids, index, type)
	{
		if(loader.item === null)
		{
			if(!__playerComponent)
				__playerComponent = controller._Component("component/VideoPlayer");
			loader.sourceComponent = __playerComponent;
			if (loader.status === Loader.Ready)
			{
				...
				if(_UT.dev !== 0) console.log(qsTr("Load video player successful"));
				return 0;
			}
			else
			{
				if(_UT.dev !== 0) console.log(qsTr("Load video player fail"));
				return -1;
			}
		}
		else
