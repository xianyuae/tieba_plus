...
Rectangle {
	...
	VideoControllerWidget{
		...
		RectWidget{
			...
			color: constants._cSurfaceColor;
			visible: width !== 0;
	...
	QtObject{
		...
		{
			if(_UT.dev !== 0) console.log("Get content -> " + data.name);
			v._Reset();
			d._Reset();
			c._Reset();
			partslist.model = [];
		...
		{
			if(_UT.dev !== 0) console.log("Play part -> " + index);
		...
		{
			if(_UT.dev !== 0) console.log("Play source -> " + url);
			if(st.isDashFormat)
			{
				_PlayExternally(url);
			}
