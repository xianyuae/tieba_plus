# TiebaLite for Nokia N9 / MeeGo 1.2 Harmattan
...
| 关注吧首页 + 一键签到 | ✅ | 前台执行，带进度条；启动只提醒不自动签 |
...
| 日志查看 | ✅ | 关于页 → 日志（HTTP/签到/错误，~/.config/TiebaLite/tieba.log） |
...
- `main.qml` — PageStackWindow + 全局 toast + 启动签到提醒
- `MainPage.qml` — 底部 4 Tab（首页/动态/消息/我的）+ 菜单
- `HomePage.qml` — 关注吧 + 一键签到（前台进度条）
...
| `db.h/cpp` | SQLite：账号/草稿/历史/搜索历史/收藏/黑名单/吧缓存/签到记录/页面离线缓存 |
...
- **日志**：`LogStore` 记录 HTTP/签到/错误到 `~/.config/TiebaLite/tieba.log`（内存保留 500 条），关于页可查看。
...
- 更新检查走 GitHub Releases API（`min09577/TiebaLite`，可自行改仓库）。
